...
import { useGetDummyTicket } from '../dummyTickets/useGetDummyTicket';
...
function generateEmailTemplate(templateName) {
  const { subject, body } = templates.find(
...
export default function SendEmail() {
  ...
  const { dummyTicket, isLoadingDummyTicket } = useGetDummyTicket(
  ...
  useEffect(() => {
    if (searchParams.get('template')) {
      ...
      const updatedBody = body
        .replace(
          '{leadPassenger}',
          capitalCase(dummyTicket?.passengers[0]?.firstName || '') || ''
        )
        ...
        <FormRow>
          ...
          <div>
            <Input
              ...
              onChange={(e) => setReservation(e.target.files[0]) || null}
            />
            {reservation && (
              <p className="text-sm mt-2">Selected: {reservation?.name}</p>
            )}
          </div>
        </FormRow>
        <PrimaryButton
